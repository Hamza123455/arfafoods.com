 let receipts = [];
  ...
  document.getElementById('configBtn').addEventListener('click', () => {
    const cfg = getConfig();
    document.getElementById('webAppUrl').value = cfg.url;
  ...
  document.getElementById('saveConfigBtn').addEventListener('click', () => {
    ...
    if (!url || !key) { alert('Please enter both the Web App URL and the Secret Key.'); return; }
  ...
  async function connectAndLoad() {
    ...
    if (!cfg.url || !cfg.key) {
      ...
      disabledHint.textContent = 'Connect to the shared data source above before adding entries.';
      return;
    }
    ...
    try {
      ...
      disabledHint.textContent = 'Changes save straight to the shared sheet and are visible on every PC.';
